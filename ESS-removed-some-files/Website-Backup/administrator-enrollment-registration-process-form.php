...
?>
	...
	<body>
  	<div class="container">
          ...
          <div align="center" style="font-size: 30px;font-style: normal;text-decoration-line: underline;"> ADMIN DASHBOARD </div>
        <br>
  	</div>
  	<div class="container">
  		<div class="row">
			...
			<form class="d-flex">
					...
					<a href="administrator-enrollment-registration-process-form.php?logout='1'"><button type='button' class='btn btn-danger'>LOGOUT </button></a>
			</form>
  			</div>
  		</div>
  	</div>
	...
	<?php
		...
		{	
			...
			if($regitem['VERIFY'] == 0)
			{
				//$createTable .= "<label type='label' class='btn btn-secondary' disabled>To be verified</label>";
				$createTable .= "<div style='color: red;font-size:20px;text-align:center;'>To be verified</div>";
			}
			if($regitem['VERIFY'] == 1)
			{
				//$createTable .= "<label type='label' class='btn btn-info' disabled>To be processed</label>";
				$createTable .= "<div style='color: orange;font-size:20px;text-align:center;'>To be processed</div>";
			}
			else if($regitem['VERIFY'] == 2)
			{
				//$createTable .= "<label type='label' class='btn btn-success' disabled>Processed</label>";
				$createTable .= "<div style='color: green;font-size:20px;text-align:center;'>Processed</div>";
			}
			//$createTable .= "<div class='btn-group' role='group' aria-label='Basic example'>FOR VERIFICATION";
