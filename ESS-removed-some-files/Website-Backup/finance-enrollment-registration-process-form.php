...
<body style="border:1px solid background: 
    ...
    <!-- Sign in  Form -->
        ...
        {   
            $student_id_arr[] = array("ID"=>$regitem['PROCESS'],"NAME"=>$regitem['NAME']);
            ...
            $getReceipt = mysqli_query($dbConn, "SELECT * FROM oc_payments WHERE registration_id = " . $regitem['VIEW'] );
            ...
            if($receipt !=null)
            {
                ...
                $createTable .= "<a href='finance-enrollment-registration-process-form.php?userid=".$regitem['VIEW']."' ><label type='label' class='btn btn-primary' onSubmit='window.location.reload()''>VIEW PAYMENT FORM</label></a>";
            }
            else
            {
            ...
            {
                $createTable .= "<td style='text-align:center;' >";
                $createTable .= "<a href='controller/finance-controller.php?prcss_id=".$regitem['VIEW']."'><button type='button' class='btn btn-warning' >SEND COE/OR </button></a>";
