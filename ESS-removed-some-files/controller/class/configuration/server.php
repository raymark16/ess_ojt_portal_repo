				...
				if($row['schlusr_password'] == md5($password))
				{
					if($row['schlusr_status'] == 1)
					{
						...
						else if($_SESSION["POSITION"] == 1)
						{
							...
							$_SESSION['ESS_USERNAME']   = $row["schlusr_username"];
							$_SESSION['ESS_FIRST_NAME'] = $row["schlusr_fname"];
							$_SESSION['ESS_LAST_NAME']	= $row["schlusr_lname"];
							$_SESSION['ESS_POSITION']	= $row["schlusr_acclvl"];
							$_SESSION['ESS_USERID']		= $row["schlusr_id"];
						}	
						else if($_SESSION["POSITION"] == 2)
						{
							
							$dashboard = 'teamleader-enrollment-registration-process-form.php';
							$_SESSION['TEAM_USERNAME']   	= $row["schlusr_username"];
							$_SESSION['TEAM_FIRST_NAME'] 	= $row["schlusr_fname"];
							$_SESSION['TEAM_LAST_NAME']		= $row["schlusr_lname"];
							$_SESSION['TEAM_POSITION']		= $row["schlusr_acclvl"];
							$_SESSION['TEAM_USERID']		= $row["schlusr_id"];
