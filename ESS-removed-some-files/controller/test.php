...
<?php
    ...
    if ($_GET['type'] == 'STUD_DETAILS'){
		...
		$qry = "SELECT `schlenrollprereg_id`,
		`schlenrollprereg_lname`,
	...
	if ($_GET['type'] == 'VIEW_DOCUMENT') {
		$reg_id = $_GET['reg_id'];
		$qry = "SELECT * FROM oc_uploaded_documents 
		WHERE document_id 
	...
	if ($_GET['type'] == 'VIEW_SPECIFIC_DOCUMENT') {
		$view_document_id = $_GET['view_document_id'];
		$qry = "SELECT document_location FROM oc_uploaded_documents 
		WHERE document_id = '$view_document_id'";
