<?php
	...
	$_SESSION['link'] = 'enrollmentregistrationform.php';

    include('controller/student_view.php');
    include('controller/student_process_verify.php');
?>
                ...
                */
				if($receipt != null)
				{
					if($document['document_id'] != $receipt['receipt_id'])
					{
						...
						echo "<a href='download.php?file=$file'><label class='btn btn-info form-control download_link' style = 'font-size: 20;'><span class='iconify' data-icon='bx:download'></span>Download File</label></a>";

						//echo "<button type='button' style='background-color: #6c6cea;' class='form-control download_link' value='".$location."'><span class='iconify' data-icon='bx:download'></span> View Document</button>";
						echo '</div>';
					}
				}
				else
